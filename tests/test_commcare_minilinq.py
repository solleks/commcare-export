...
class TestCommCareMiniLinq(unittest.TestCase):
    ...
    def test_eval(self):
        ...
        client = MockCommCareHqClient({
            'form': [
                (
                    {'limit': 1000, 'filter': 'test1', 'order_by': ['server_modified_on', 'received_on', 'indexed_on']},
                    [1, 2, 3],
                ),
                (
                    {'limit': 1000, 'filter': 'test2', 'order_by': ['server_modified_on', 'received_on', 'indexed_on']},
                    [
                        { 'x': [{ 'y': 1 }, {'y': 2}] },
                        { 'x': [{ 'y': 3 }, {'z': 4}] },
                        { 'x': [{ 'y': 5 }] }
                ...
                (
                    {'limit': 1000, 'filter': 'laziness-test', 'order_by': ['server_modified_on', 'received_on', 'indexed_on']},
                    (i if i < 5 else die('Not lazy enough') for i in range(12))
                ),
                (
                    {'limit': 1000, 'cases__full': 'true', 'order_by': ['server_modified_on', 'received_on', 'indexed_on']},
                    [1, 2, 3, 4, 5]
                ),
            ],
            ...
            'case': [
                (
                    {'limit': 1000, 'type': 'foo', 'order_by': ['server_date_modified', 'indexed_on']},
                    [
                        { 'x': 1 },
                        { 'x': 2 },
                        { 'x': 3 },
